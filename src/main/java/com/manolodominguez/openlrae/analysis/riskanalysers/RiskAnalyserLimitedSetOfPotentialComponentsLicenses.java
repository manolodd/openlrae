...
public class RiskAnalyserLimitedSetOfPotentialComponentsLicenses extends AbstractRiskAnalyser {
    ...
    private Logger logger = LoggerFactory.getLogger(RiskAnalyserLimitedSetOfPotentialComponentsLicenses.class);

    // NOTE: THIS HAS TO BE MODIFIED TO TAKE INTO ACCOUNT ALSO ALL SUPPORTED LINKS
    ...
    public RiskAnalysisResult getRiskAnalysisResult() {
        ...
        for (SupportedLicenses potentialComponentLicense : SupportedLicenses.values()) {
            for (SupportedLinks potentialLink : SupportedLinks.values()) {
                if ((potentialComponentLicense != SupportedLicenses.FORCED_AS_PROJECT_LICENSE) && (potentialComponentLicense != SupportedLicenses.UNDEFINED)) {
